...
export default function WhiteLabelAffiliateDashboard() {
  ...
  // CRITICAL FIX: Use proper array queryKey and explicit queryFn to avoid caching issues
  const { data: whiteLabelInfo, isLoading: isWhiteLabelLoading, error: whiteLabelError } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/white-labels/by-id/${user!.whiteLabelId}`);
      if (!response.ok) {
  ...
  // Debug logging
  console.log('🔍 White Label Affiliate Dashboard Debug:', {
    userId: user?.id,
    userRole: user?.role,
  ...
  useEffect(() => {
    if (user?.whiteLabelId && isAuthenticated) {
      console.log('🔄 Invalidating white label cache for fresh domainPath fetch');
  ...
  const copyAffiliateLink = (referralCode?: string) => {
    ...
    // Use whiteLabelInfo.domainPath as the canonical domain source
    const domainPath = whiteLabelInfo?.domainPath;
    if (!domainPath) {
      toast({
        title: "Error",
        description: "Domain path not available",
        variant: "destructive",
      });
      return;
    }
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent>
                  {isWhiteLabelLoading || referralCodeLoading ? (
                    <div className="flex items-center justify-center py-8">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                    </div>
                  ) : !whiteLabelInfo?.domainPath ? (
                    <div className="px-3 py-2 bg-red-50 border border-red-200 rounded-md text-sm text-red-600">
                      Domain path not available. Please contact support.
                    </div>
                          ...
                          <>
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                              <div className="flex-1 w-full px-3 py-2 bg-green-50 border border-green-200 rounded-md font-mono text-sm break-all">
                                {window.location.origin}/{whiteLabelInfo.domainPath}?{referralCodeData.referralCode}
                              </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                        <div className="flex-1 w-full flex flex-col sm:flex-row items-stretch border rounded-md">
                          <div className="px-3 py-2 bg-gray-50 text-gray-600 text-sm border-b sm:border-b-0 sm:border-r">
                            {window.location.origin}/{whiteLabelInfo.domainPath}?
                          </div>
                          <Input
              ...
              {/* QR Code Generator */}
              {referralCodeData?.hasReferralCode && whiteLabelInfo?.domainPath && (
                <QRCodeGenerator
                  url={`${window.location.origin}/${whiteLabelInfo.domainPath}?${referralCodeData.referralCode}`}
                  title="Referral URL"
                  size={180}
                />
              )}
