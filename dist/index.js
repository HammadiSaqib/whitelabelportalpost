...
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async trackReferralClickSimple(referralCode, ipAddress, userAgent) {
        const [click] = await db.insert(referralClicks).values({
          referralCode,
      ...
      async getEndUsersByWhiteLabelDetailed(whiteLabelId2) {
      ...
      async getPlanCategoriesFromPlan(planId) {
      ...
      async getPlanProductsFromPlan(planId) {
        const plan = await this.getPlan(planId);
        if (!plan) {
          return [];
        }
      ...
      async getPlanPurchasersDetailed(planId) {
        try {
          const purchasers = await db.select({
            id: purchaseHistory.id,
            amount: purchaseHistory.amount,
