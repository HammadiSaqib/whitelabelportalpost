...
export class DatabaseStorage implements IStorage {
  ...
  async trackReferralClickSimple(referralCode: string, ipAddress?: string, userAgent?: string): Promise<ReferralClick> {
    const [click] = await db
      .insert(referralClicks)
      .values({
        referralCode,
  ...
  // End-user management for white-label clients
  async getEndUsersByWhiteLabelDetailed(whiteLabelId: number): Promise<{
    id: string;
    email: string | null;
    firstName: string | null;
    lastName: string | null;
  ...
  async getPlanCategoriesFromPlan(planId: number): Promise<Category[]> {
  ...
  async getPlanProductsFromPlan(planId: number): Promise<Product[]> {
    const plan = await this.getPlan(planId);
    if (!plan) {
      return [];
    }
  ...
  async getPlanPurchasersDetailed(planId: number): Promise<any[]> {
    try {
